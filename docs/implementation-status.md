...
- **Kubernetes**：EndpointSlice 端点跳过 `conditions.serving: false`（与 `ready: false` 一致策略）；`doctor-json-schema`/运维手册补充说明。解析路径支持 `RUST_LOG=service_router::registry::k8s=debug` 区分 Core Endpoints 与 EndpointSlice 回退（见 `operations-runbook` §6）。
